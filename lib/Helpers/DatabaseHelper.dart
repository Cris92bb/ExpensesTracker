...
''');
  ...
  Future<SingleExpense?> getExpense(int id) async { // Return type is nullable
    // Ensure db is not null
    if (db == null) return null; 
    List<Map<String, Object?>> maps = await db!.query(tableExpenses,
        columns: [columnId, columnDate, columnAmmount,columnIcon,columnNote],
        where: "$columnId = ?",
        whereArgs: [id]);
    if (maps.isNotEmpty) { // Use isNotEmpty
  ...
  Future<List<SingleExpense>> getAllExpenses() async { // Return non-nullable list
    // Ensure db is not null
    if (db == null) return []; 
    List<Map<String, Object?>> maps = await db!.query(tableExpenses,
        columns: [columnId, columnDate, columnAmmount,columnIcon,columnNote]);
